...
class GatedLinearUnit(nn.Module):
    """Enhanced Gated Linear Unit for improved feature learning"""
...
class AdaptiveTemporalEmbedding(nn.Module):
    """Advanced temporal embedding with adaptive time-of-day and day-of-week features"""
...
class MultiscaleTemporalConvolution(nn.Module):
    """Enhanced temporal convolution with multiple dilations for multi-scale dependency capture"""
    def __init__(self, features=128, layer=4, length=12, dropout=0.1):
        super(MultiscaleTemporalConvolution, self).__init__()
...
class MemoryAugmentedSpatialAttention(nn.Module):
    """Memory-augmented spatial attention with adaptive graph learning"""
    def __init__(self, device, d_model, head, num_nodes, seq_length=1, dropout=0.1, memory_size=4):
        super(MemoryAugmentedSpatialAttention, self).__init__()
        ...
        self.q = ConvolutionalBlock(d_model)
        self.k = ConvolutionalBlock(d_model)
        self.v = ConvolutionalBlock(d_model)
        self.concat = ConvolutionalBlock(d_model)
        
        # adaptive memory banks
...
class MSMTEncoderBlock(nn.Module):
    """MSMT Encoder Block with memory-augmented attention and gated processing"""
    def __init__(self, device, d_model, head, num_nodes, seq_length=1, dropout=0.1, memory_size=4):
        super(MSMTEncoderBlock, self).__init__()
        assert d_model % head == 0
        self.attention = MemoryAugmentedSpatialAttention(device, d_model, head, num_nodes, seq_length, dropout, memory_size)
...
class UncertaintyAwareOutputHead(nn.Module):
    """Multi-resolution output module with uncertainty quantification"""
    ...
    def forward(self, x):
        ...
        # integrate fine/coarse predictions
...
class MSMT(nn.Module):
    """
    Memory-augmented Spatio-temporal Multi-scale Transformer (MSMT)
    
    An enhanced spatio-temporal forecasting model with:
    ...
    def __init__(
        ...
        # adaptive time size selection based on dataset characteristics
        ...
        self.temporal_embedding = AdaptiveTemporalEmbedding(time, channels)
        self.temporal_conv      = MultiscaleTemporalConvolution(channels, layer=4, length=input_len)
        self.start_conv         = nn.Conv2d(input_dim, channels, kernel_size=(1,1))
        self.network_channel    = channels*2

        self.spatial_encoder = MSMTEncoderBlock(
            device, self.network_channel, head=1,
            num_nodes=num_nodes, seq_length=input_len,
            dropout=dropout, memory_size=memory_size
        )
        self.feature_combiner   = nn.Conv2d(self.network_channel, self.network_channel, kernel_size=(1,1))
        self.output_head        = UncertaintyAwareOutputHead(self.network_channel, output_len, num_nodes)
    ...
    def forward(self, history_data, return_uncertainty=False):
        """
        Forward pass of MSMT model
        
        Args:
            history_data: Historical input data [B, C_in, N, T]
            return_uncertainty: Whether to return uncertainty estimates
            
        Returns:
            main_pred: Primary predictions [B, output_len, N, 1]
        ...
        x = self.temporal_conv(x)       # [B, channels, N, T]
        
        # Temporal embedding integration
        tem_emb = self.temporal_embedding(history_data)  # [B, channels, N, T]
        
        # Spatio-temporal feature fusion
        data_st = torch.cat([x, tem_emb], dim=1)  # [B, 2*channels, N, T]
        data_st = self.spatial_encoder(data_st) + self.feature_combiner(data_st)
        
        # Multi-resolution prediction
        main_pred, fine_pred, coarse_pred, uncertainty = self.output_head(data_st)
